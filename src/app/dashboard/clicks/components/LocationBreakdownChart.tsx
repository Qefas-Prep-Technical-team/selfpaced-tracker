  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between px-2">
        ...
        <span className="text-slate-400 text-xs font-medium uppercase tracking-wider">Top Markets</span>
      </div>
...
/>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-white/10 pt-4">
          {locations.slice(0, 4).map((loc: any, index: number) => (
            <div key={loc.name} className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="size-2 rounded-full" style={{ backgroundColor: BAR_COLORS[index] }} />
                <span className="text-xs font-medium text-slate-600 dark:text-slate-400">{loc.name}</span>
              </div>
